...
async function generateProgressInsight(force){
  ...
  if(supabaseClient){
    ...
    const invokePromise = supabaseClient.functions.invoke('ai-chat', {
      body: { message: prompt, chapterTitle: 'Bilan de progression', chapterSummary: statsSummary, history: [] }
    });
    /* filet de sécurité : si l'appel reste en attente indéfiniment, on ne bloque pas
       le bilan pour autant — voir le même filet sur le chat de Nova plus bas. */
    const timeout = new Promise(resolve => setTimeout(() => resolve({ timedOut: true }), 12000));
    Promise.race([invokePromise, timeout]).then((result) => {
      if(result && result.timedOut){ console.warn('[ai-chat/progress] délai dépassé, repli local'); finish(localProgressInsight(lastBadgeStats)); return; }
...
async function sendPrivateMessage(){
  ...
  await withLoading(btn, async () => {
    try{
      ...
      if(error){ console.error('[sendPrivateMessage]', error); input.value = body; return; }
      /* affichage immédiat de la conversation à jour : on ne compte plus uniquement
         sur l'écho du temps réel (subscribePrivateThreadRealtime), qui pouvait arriver
         en retard et obligeait à rouvrir la boîte pour voir son propre message. */
      await loadPrivateThreadMessages();
...
/* Ajoute un message dans le salon commun, sauf s'il y est déjà (id déjà présent dans
   le DOM). Centralisé ici pour être appelé à la fois : (1) juste après l'envoi, tout
   de suite, sans attendre le temps réel — c'est ce qui manquait et qui obligeait à
   rouvrir la boîte pour voir son propre message ; et (2) par l'abonnement temps réel
   pour les messages des autres (et pour le sien si jamais l'écho arrive quand même,
   auquel cas il est ignoré grâce à la vérification d'id, pas dupliqué). */
function appendChatRoomMessageOnce(m){
  const host = document.getElementById('chatRoomBody');
  if(!host || document.getElementById('chatmsg-' + m.id)) return;
  const wasAtBottom = host.scrollTop + host.clientHeight >= host.scrollHeight - 40;
  host.insertAdjacentHTML('beforeend', renderChatRoomMessage(m));
...
function subscribeChatRoomRealtime(){
  ...
  chatRoomChannel = supabaseClient
    ...
    .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, (payload) => {
      ...
      appendChatRoomMessageOnce(payload.new);
    })
...
async function sendChatRoomMessage(){
  ...
  await withLoading(btn, async () => {
    try{
      ...
      if(error){ console.error('[sendChatRoomMessage]', error); input.value = body; return; }
      /* affichage immédiat : on n'attend plus uniquement l'écho du temps réel
         (parfois en retard, parfois manqué), qui obligeait à rouvrir la boîte. */
      appendChatRoomMessageOnce(data);
...
/* Les tableaux « .mini-table » écrits dans le contenu des chapitres n'ont pas de
   conteneur autour d'eux. Sur mobile, un tableau à plusieurs colonnes (ex. tableaux
   de constantes, de substituants...) est alors plus large que l'écran ; comme
   .course-panel est en overflow:hidden (nécessaire pour le filigrane et les vagues
   décoratives), le texte du tableau était purement et simplement coupé au lieu de
   défiler. On enveloppe donc chaque <table class="mini-table"> dans un
   <div class="table-wrap"> (overflow-x:auto défini dans le CSS) juste avant
   l'injection dans le DOM, sans toucher aux fichiers de chunks. */
function wrapWideTables(html){
  if(!html) return html;
  return html.replace(/<table class="mini-table">[\s\S]*?<\/table>/g, m => `<div class="table-wrap">${m}</div>`);
}
function openChapitreNow(domainKey, matiereName, chapterIndex){
  ...
  if(mathChapter){
    document.getElementById('coursePanelAuthored').innerHTML = wrapWideTables(mathChapter.bodyHtml);
    document.getElementById('chapAuthoredExtra').innerHTML = wrapWideTables(mathChapter.extraHtml);
    ...
    document.getElementById('coursePanelAuthored').innerHTML = wrapWideTables(ORIGINAL_AUTHORED_BODY);
    document.getElementById('chapAuthoredExtra').innerHTML = wrapWideTables(ORIGINAL_AUTHORED_EXTRA);
...
async function requestRemediationExplanation(){
  ...
  if(supabaseClient){
    ...
    /* filet de sécurité : même principe que pour le chat de Nova — ne jamais laisser
       « Nova prépare une explication… » tourner indéfiniment si l'appel reste bloqué. */
    const timeout = new Promise(resolve => setTimeout(() => resolve({ timedOut: true }), 12000));
    Promise.race([invokePromise, timeout]).then((result) => {
      if(result && result.timedOut){ finish(`Reprenons différemment : ${pointsList}. Relis attentivement le passage du cours qui correspond, souvent une seule relecture ciblée suffit à débloquer ce genre de point.`); return; }
      const { data, error } = result;
      if(!error && data && data.reply){ finish(jsEscHtmlKeepLines(data.reply)); }
...
/* ===================================================================
   ...
   comptes, progression, Nova). Le contenu des cours vit dans <id>.js
   (un fichier par matière, à la racine du repository, à côté d'index.html) et
   n'est chargé qu'au premier accès à un chapitre de cette matière — voir
   loadChunk() et openChapitre() ci-dessous.

   ➜ Pour ÉDITER un chapitre : ouvrir <id>.js (id = nom du registre en
     minuscules : THD -> thd.js). Rien à changer ici.
   ➜ Pour AJOUTER UNE NOUVELLE MATIÈRE : créer <id>.js (même schéma que
...
/* ===================================================================
   INDEX DES CHUNKS : « Domaine|Matière » -> fichier <id>.js (à la racine)
=================================================================== */
const CHUNK_INDEX = {
...
function injectChunkScript(id, attempt){
  return new Promise((resolve, reject) => {
    ...
    s.src = id + '.js' + (attempt ? '?r=' + attempt + '-' + Date.now() : '');
    s.async = true;
...
async function fetchChunkScript(id){
  const res = await fetch(id + '.js?r=f' + Date.now(), { cache: 'no-store' });
...
async function diagnoseChunk(id){
  ...
  try{ const r = await fetch(id + '.js', { method: 'HEAD', cache: 'no-store' }); return { status: r.status }; }
...
function chunkErrorMessage(id, diag){
  ...
  if(diag && diag.status === 404) return 'Fichier de cours introuvable sur le serveur (' + id + '.js). Ce fichier doit être publié à côté d\'index.html.';
  if(diag && diag.status >= 400) return 'Le serveur a répondu ' + diag.status + ' pour ' + id + '.js. Réessaie dans un instant.';
...
function prefetchChunk(id){
  ...
  try{
    ...
    l.rel = 'prefetch'; l.as = 'script'; l.href = id + '.js';
...
function sendDemo(){
  ...
  if(supabaseClient){
    ...
    /* Filet de sécurité : sans ceci, si la fonction distante ne répond jamais
       (serveur qui ne timeout pas lui-même, requête qui reste en attente...),
       ni .then() ni .catch() ne se déclenchent -> « Nova réfléchit… » restait
       affiché indéfiniment et le chat semblait ne plus répondre du tout. Passé
       ce délai, on bascule sur le mentor local, exactement comme en cas d'erreur
       réseau classique — Nova répond toujours, même si la fonction distante est
       bloquée ou en panne. */
    const timeout = new Promise(resolve => setTimeout(() => resolve({ timedOut: true }), 12000));
    Promise.race([invokePromise, timeout]).then((result) => {
      if(result && result.timedOut){
        console.warn('[ai-chat] délai dépassé (12 s), repli sur le mentor local');
