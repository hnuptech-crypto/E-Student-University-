...
function buildHistoryState(view){
  return {
    view,
    domain: currentDomainKtx,
    matiereCtx: currentMatiereCtx ? { domain: currentMatiereCtx.domain, index: currentMatiereCtx.index } : null,
    chapterCtx: currentChapterCtx ? { domain: currentChapterCtx.domain, matiere: currentChapterCtx.matiere, index: currentChapterCtx.index } : null
  };
}
function goToHistoryState(state){
  if(!state){ showView('accueil'); return; }
  isRestoringHistory = true;
  try{
    if(state.view === 'chapitre' && state.chapterCtx){ openChapitre(state.chapterCtx.domain, state.chapterCtx.matiere, state.chapterCtx.index); }
    else if(state.view === 'matiere' && state.matiereCtx){ openMatiereDetail(state.matiereCtx.domain, state.matiereCtx.index); }
    else if(state.view === 'domaine' && state.domain){ openDomaine(state.domain); }
    else{ showView(state.view || 'accueil'); }
  } finally { isRestoringHistory = false; }
}
window.addEventListener('popstate', (e) => { goToHistoryState(e.state); });
...
function showView(view){
  if(!isRestoringHistory){
    try{ history.pushState(buildHistoryState(view), '', '#'+view); }catch(e){ console.warn('[history]', e); }
  }
...
const DOMAINS = [
  ...
  {key:'Informatique', symbol:'In', no:'04', accent:'var(--violet)'},
  ...
  DOMAINS.forEach(d => {
    ...
    grid.insertAdjacentHTML('beforeend', `
      <div class="element-card reveal in" data-name="${d.key}" data-chapters="${matiereNames.replace(/"/g,'&quot;')}" style="--accent:${d.accent};" onclick="openDomaine('${d.key}')">
        <div class="tag">
          <div><div class="atomic-no">${d.no}</div><div class="symbol">${d.symbol}</div></div>
...
function openDomaine(domainKey){
  ...
  const matieres = getMatieres(selectedLevel, domainKey);
  document.getElementById('domaineSymbol').textContent = dom.symbol;
...
function openMatiereDetail(domainKey, matiereIndex){
  ...
  document.getElementById('matiereSymbol').textContent = dom.symbol;
...
function filterSubjects(){
  ...
  results.innerHTML = hits.map(h => `
    <div class="search-hit" style="--accent:${h.accent};" onclick="jumpToMatiere('${h.level}','${h.domain}',${h.idx})">
      <span class="search-hit-symbol">${h.symbol}</span>
...
function loadChunk(id){
  ...
  chunkPromises[id] = new Promise((resolve, reject) => {
    ...
    s.src = 'chunks/' + id + '.js';
    s.async = true;
    s.onload = () => { chunkState[id] = 'ready'; resolve(id); };
    s.onerror = () => { delete chunkPromises[id]; s.remove(); reject(new Error('chunk ' + id + ' introuvable')); };
...
function openChapitre(domainKey, matiereName, chapterIndex){
  ...
  return loadChunk(id).then(() => {
    ...
    console.warn('[chunk]', err);
    esuStatus('Impossible de charger ce cours. Vérifie ta connexion puis réessaie.', 5000);
  });
}
...
try{ history.replaceState(buildHistoryState('accueil'), '', '#accueil'); }catch(e){ console.warn('[history]', e); }
